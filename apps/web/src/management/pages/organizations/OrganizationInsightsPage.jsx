...
function PhotoLightbox({ photos, index, onClose, onPrev, onNext }) {
  ...
  const src      = photo.thumbnail?.md ?? `/api/galleries/${photo.galleryId}/photos/${encodeURIComponent(filename)}/preview`;
...
export default function OrganizationInsightsPage() {
  ...
  return (
    <AdminPage title={t('tab_insights')} maxWidth="100%">
      <PhotoLightbox
        photos={lightboxPhotos} index={lightboxIdx}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(90px, 1fr))', gap: '0.5rem' }}>
                  {bin.photos.map((photo, idx) => (
                    <div key={photo.filename}
                      ...
                      <img
                        src={photo.thumbnail?.sm ?? `/api/galleries/${photo.galleryId}/photos/${encodeURIComponent(photo.filename)}/preview`}
                        alt={photo.filename}
